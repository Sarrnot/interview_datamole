...
# Runs typecheck (tsc --noEmit) then the full test suite over the entire client
# package — both too slow/broad to run per edit. Lint and file-scoped tests are
...
if ! output=$(cd "$root" && pnpm --filter ./client test 2>&1); then
    fail "Tests" "$output"
fi
