...
-   **Shared `Todo` shape**: declared client-side in `client/src/api/todos/`, mirrored by rows in `server/db.json`. No shared types package — keep both in sync by hand. The client validates every response against `TodoSchema` at the transport boundary.
...
-   **Keep CLAUDE.md files in sync**: when a change makes any of these docs stale (this file, `client/CLAUDE.md`, `server/CLAUDE.md`, etc.), update the affected file in the same change.
